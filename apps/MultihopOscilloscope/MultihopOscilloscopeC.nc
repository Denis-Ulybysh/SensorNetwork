...
/*
uld Search tos.lib.net.ctp.CtpInfo
Methods to inject packet loss
1) Increase distance between nodes (from 2 to 2.1 etc meters)
2) Probabilistic drop - see interface Intercept. if it returns false packet will be dropped and not sent to the neighbor
*/
#include "Timer.h"
...
module MultihopOscilloscopeC @safe(){
  uses {
    ...
    interface SplitControl as SerialControl;  //no need now
    interface StdControl as RoutingControl;
    
    // Interfaces for communication, multihop and serial:
    interface Send;
    interface Receive as Snoop;  //can sniffer packets intended for other nodes
    interface Receive;
...
implementation {
  ...
  event void SerialControl.startDone(error_t error) {
    ...
    if (local.id % 500 == 0)       //nodeID=0,500,1000... is a root
      call RootControl.setRoot();  //all the packets will be sent to root node
  ...
  //
  event message_t*
  //this receive will work only for root node. Intermediate nodes are handled by CTP
